...
个人自用仓库：用代理软件的 HTTPS 解密功能，把 Apple 地图定位改到指定坐标。

仓库地址：https://github.com/SuperMalow/location-custom

> 📖 **新手直接看这篇** → [**小白保姆级图文教程**](使用教程.md)（一步步教你安装、配置、生效，含常见问题排查）

## 功能概览

- **多平台支持** — Shadowrocket / Surge / Loon / Quantumult X / Stash，免编译、免开发者账号
- **蜂窝基站坐标修改** — 除 Wi-Fi 热点外，一并处理 CellTower（字段 22/24）
- **多响应格式兼容** — 自动检测 Apple 回应封装（ARPC / synthetic / marker / bare）
- **运动状态伪造** — 改写 motionActivityType / motionActivityConfidence
- **可选地图选点** — `location-picker/` 网页工具，配合 `configUrl` 远程下发坐标
...
| 软件 | 文件 | 导入方法 | 状态 |
|------|------|---------|------|
| Shadowrocket（小火箭） | `location-custom.sgmodule` | 配置 → 右上角 + | ✅ 实测通过 |
| Surge | `location-custom-surge.sgmodule` | 首页 → 模块 → 安装新模块 | ✅ 实测通过 |
| Loon | `location-custom.lnplugin` | 设置 → 插件 → 添加插件 | ✅ 实测通过 |
| Quantumult X | `location-custom.snippet` | 设置 → 重写 → 添加 | 🟡 待测试 |
| Stash | `location-custom.stoverride` | 覆写 → 安装覆写 | ✅ 实测通过 |
...
1. 导入 `location-custom.lnplugin` 后，在 **设置 → 插件** 里打开插件配置页
...
5. 改坐标后关开定位；调试打开 **调试日志**，在 Loon 日志搜 `location-custom`
...
location-custom.js              # 核心脚本（四平台共用）
location-custom-qx.js           # QX 专用
location-custom-config.json     # 配置样板
使用教程.md                     # 小白保姆级图文教程
